 ...
 */
export function assertNotAuthError(status, context) {
    if (status !== 401 && status !== 403)
        return;
    if (isAuthEnabled()) {
        throw new Error(`${context}: opencode server rejected credentials (HTTP ${status}). ` +
    ...
    throw new Error(`${context}: opencode server requires authentication (HTTP ${status}) but ` +
