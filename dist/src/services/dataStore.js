...
export function addProject(alias, path) {
    const data = loadData();
...
export function setChannelAgent(channelId, agent) {
    const data = loadData();
    const existing = data.bindings.findIndex(b => b.channelId === channelId);
    if (existing >= 0) {
        data.bindings[existing].agent = agent || undefined;
        saveData(data);
        return true;
    }
    return false;
...
export function getChannelProjectPath(channelId) {
...
export function setThreadSession(session) {
    const data = loadData();
...
export function updateThreadSessionLastUsed(threadId) {
    const data = loadData();
...
export function setWorktreeMapping(mapping) {
    const data = loadData();
...
export function getWorktreeMapping(threadId) {
    const data = loadData();
    return data.worktreeMappings?.find(m => m.threadId === threadId);
}
export function getWorktreeMappingByBranch(projectPath, branchName) {
    const data = loadData();
    return data.worktreeMappings?.find(m => m.projectPath === projectPath && m.branchName === branchName);
}
export function removeWorktreeMapping(threadId) {
    const data = loadData();
    if (!data.worktreeMappings)
        return false;
...
export function getAllWorktreeMappings() {
    return loadData().worktreeMappings ?? [];
}
export function getWorktreeMappingsByProject(projectPath) {
    const data = loadData();
...
export function setProjectAutoPassthrough(alias, enabled) {
    const data = loadData();
    const project = data.projects.find(p => p.alias === alias);
    if (!project)
        return false;
...
export function getProjectAutoPassthrough(alias) {
    const project = getProject(alias);
