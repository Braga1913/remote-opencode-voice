import { ActionRowBuilder, ComponentType, EmbedBuilder, MessageFlags, SlashCommandBuilder, StringSelectMenuBuilder, } from 'discord.js';
    ...
    async execute(interaction) {
        ...
        if (subcommand === 'list') {
            ...
            try {
                ...
                const allSessionIds = Array.from(new Set([...activeSessions.map((s) => s.id), ...persistedSessions.map((s) => s.sessionId)]));
                ...
                await interaction.editReply({
                    content: `❌ Failed to list sessions: ${error.message}`,
                });
            }
            return;
        }
        if (subcommand === 'attach') {
            ...
            try {
                ...
                collector.on('collect', async (i) => {
                    ...
                    const existingMapping = allSessions.find((s) => s.sessionId === sessionId && s.threadId !== threadId);
                ...
                await interaction.editReply({
                    content: `❌ Failed to attach session: ${error.message}`,
                });
            }
            return;
        }
        ...
        if (subcommand === 'info') {
            ...
            try {
                ...
                const embed = new EmbedBuilder()
                    ...
                    .addFields({ name: 'Session ID', value: `\`${currentSession.sessionId}\`` }, { name: 'Project Path', value: `\`${projectPath}\`` }, { name: 'Port', value: `\`${port}\``, inline: true }, { name: 'Status', value: isAlive ? 'alive' : 'dead', inline: true }, {
                    name: 'Created At',
                ...
                await interaction.editReply({
                    content: `❌ Failed to read session info: ${error.message}`,
                });
            }
        }
    },
