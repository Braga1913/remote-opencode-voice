...
import { setBotConfig, getBotConfig, hasBotConfig, addAllowedUserId, setOpenAIApiKey, setSonioxApiKey } from '../services/configStore.js';
...
export async function runSetupWizard() {
    ...
    // Step 6b (optional): Soniox STT for Voice Chat
    const enableSoniox = await p.confirm({
        message: 'Would you like to enable Soniox STT for Voice Chat? (requires Soniox API key)',
        initialValue: false,
    });
    if (p.isCancel(enableSoniox)) {
        p.cancel('Setup cancelled.');
        process.exit(0);
    }
    if (enableSoniox) {
        const sonioxKey = await p.password({
            message: 'Enter your Soniox API key:',
            validate: (value) => {
                if (!value)
                    return 'API key is required';
                if (value.length < 20)
        ...
        if (p.isCancel(sonioxKey)) {
            p.cancel('Setup cancelled.');
            process.exit(0);
        }
        setSonioxApiKey(sonioxKey);
        p.log.success('Soniox API key saved. Voice Chat STT enabled!');
    }
