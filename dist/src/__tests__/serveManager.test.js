    ...
    describe("waitForReady", () => {
        ...
        describe("OPENCODE_SERVER_PASSWORD auth", () => {
            ...
            it("fails fast with a clear error when readiness probe returns 401", async () => {
                ...
                await expect(serveManager.waitForReady(14097, 5000)).rejects.toThrow(/rejected by the opencode server/i);
                vi.useFakeTimers();
            });
            it("fails fast with a clear error when readiness probe returns 401 and password is unset", async () => {
                ...
                await expect(serveManager.waitForReady(14097, 5000)).rejects.toThrow(/OPENCODE_SERVER_PASSWORD is not set/i);
                vi.useFakeTimers();
            });
        });
    });
