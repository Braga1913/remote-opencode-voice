...
const dataStoreMock = vi.hoisted(() => {
    ...
    return {
        ...
        getThreadSession: vi.fn((threadId) => threadSessions.get(threadId)),
        setThreadSession: vi.fn((session) => {
            threadSessions.set(session.threadId, session);
        }),
        updateThreadSessionLastUsed: vi.fn((threadId) => {
            const session = threadSessions.get(threadId);
            if (session) {
                session.lastUsedAt = Date.now();
            }
        }),
        clearThreadSession: vi.fn((threadId) => {
...
import { createSession, sendPrompt, validateSession, getSessionInfo, listSessions, abortSession, ensureSessionForThread, getSessionForThread, setSessionForThread, clearSessionForThread, setSseClient, getSseClient, clearSseClient, } from "../services/sessionManager.js";
...
describe("SessionManager", () => {
    ...
    describe("createSession", () => {
        ...
        it("should throw error if HTTP request fails", async () => {
            ...
            await expect(createSession(3000)).rejects.toThrow("Failed to create session: 500 Internal Server Error");
        });
    ...
    describe("sendPrompt", () => {
        ...
        it("should throw error if HTTP request fails", async () => {
            ...
            await expect(sendPrompt(3000, "ses_invalid", "test")).rejects.toThrow("Failed to send prompt: 404 Not Found — Not Found");
        });
    });
