...
function makeInteraction(channelId, isThread = false, parentId) {
    return {
        channelId,
        channel: isThread
...
describe('/autocode', () => {
    ...
    it('resolves to the parent channel binding when invoked from a thread', async () => {
        ...
        vi.mocked(dataStore.getChannelBinding).mockImplementation((id) => id === 'parent-channel' ? 'myproj' : undefined);
        vi.mocked(dataStore.getProjectAutoPassthrough).mockReturnValue(false);
        vi.mocked(dataStore.setProjectAutoPassthrough).mockReturnValue(true);
        await autocode.execute(interaction);
...
describe('auto-passthrough seeding via /work', () => {
    beforeEach(() => {
        vi.clearAllMocks();
        vi.mocked(worktreeManager.sanitizeBranchName).mockImplementation((b) => b);
    ...
    async function runWork() {
        ...
        const interaction = {
            ...
            options: { getString: (n) => (n === 'branch' ? 'feat-x' : null) },
            reply: vi.fn().mockResolvedValue(undefined),
